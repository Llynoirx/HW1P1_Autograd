...
def matmul_backward(grad_output, a, b):
    ...
    a_grad = np.matmul(grad_output, b.T)
    b_grad = np.matmul(a.T, grad_output)
...
def div_backward(grad_output, a, b):
    ...
    a_grad = grad_output * (np.ones(b.shape)/b)
...
def SoftmaxCrossEntropy_backward(grad_output, pred, ground_truth):
    ...
    return grad_output * (pred-ground_truth)
